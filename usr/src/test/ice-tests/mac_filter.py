...
def main() -> None:
    ...
    assert "ice_gld_fltr_init" not in gld
    ...
    gld_construct = gld_set.index("ice_fltr_entry_init(&e,")
    assert gld_set.count("ice_fltr_entry_init(&e,") == 1
    assert gld_construct < gld_set.index("ice_add_mac(hw, &m_list)")
    assert gld_construct < gld_set.index("ice_remove_mac(hw, &m_list)")
    assert gld.count("ice_fltr_entry_init(") == 1
    ...
    assert "void\nice_fltr_entry_init(" in vsi
    assert "static void\nice_fltr_entry_init(" not in vsi
